...
def cobraPhaseII(cobra):
    ...
    constraintSurrogates = None # have them as variables on the global level of cobraPhaseII
    fitnessSurrogate = None # such that inner trainSurrogates() can access them
    fitnessSurrogate1 = None # built model according to the original value of the fitness values
    fitnessSurrogate2 = None # built model according to the plog transformed of the fitness values
    ...
    def updateInfoAndCounters(cobra, xNew, xNewEval, newNumViol, newMaxViol, phase):
        ...
        verboseprint(cobra['verbose'], False, phase+' '+str(len(cobra['A']))+' '+str(xNewEval[0])+' '+str(newMaxViol))
    ...
    def trainSurrogates(cobra):
        ...
        if cobra['RBFmodel'] == 'cubic':
            global constraintSurrogates
            constraintSurrogates = trainCubicRBF(A,Gres,squares=cobra['squaresC'],rho=cobra['RBFrho'])
            global fitnessSurrogate
        ...
        verboseprint(cobra['verbose'],False,"Finished "+str(time.time()-ptm)+" sec")
    ...
    while n < cobra['feval']:
        ...
        if cobra['seqOptimizer']=='COBYLA':            
            cons = []
            gCOBRApartial = partial(gCOBRA, cobra=cobra, ro=ro, 
                                    constraintPrediction=constraintPrediction, 
                                    constraintSurrogates=constraintSurrogates, 
                                    EPS=EPS, fn=fn)
            cons.append({'type':'ineq','fun':gCOBRApartial})
            ...
            subProb2partial = partial(subProb2, cobra=cobra, fitnessSurrogate=fitnessSurrogate)
            subMin = optimize.minimize(subProb2partial, xStart, constraints=cons, options=opts, method='COBYLA')
        ...
        cobra['predC'] = np.vstack((cobra['predC'], interpRBF(np.asmatrix(xNew), constraintSurrogates)))
...
def subProb2(x, cobra=None, fitnessSurrogate=None):
    if np.any(np.isnan(x)):
        return np.inf
    if cobra['trueFuncForSurrogates']:
        y = cobra['fn'](x)[0]
...
def gCOBRA(x, cobra=None, ro=None, constraintPrediction=None, constraintSurrogates=None, EPS=None, fn=None):
    x = np.matrix(x)
    ...
    if cobra['trueFuncForSurrogates']:
        constraintPrediction = fn(x)[-1]+EPS**2
