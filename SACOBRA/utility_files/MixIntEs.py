...
class MixedIntegerOptimizer:
    def __init__(self, obj_func, max_eval=np.inf, lower=None, upper=None, minimize=True, mu=100, lambda_=30, integer_indices = [], verbose=False, constraints_fun = None):
    ...
    def quadratic_penalty(self, constraints, mu = 1000):
        # print("constraints", np.array(constraints))
    ...
    def evaluate(self, population):
        ...
        for i, individual in enumerate(population):
            ...
            penalty = self.quadratic_penalty(constraints_val)
...
def G05_cons(x):
    ...
    return np.array([ g1, g2, h31, h32, h41, h42, h51, h52])
def run_05():
    lower_bounds = np.array([0, 0, -0.55, -0.55])
    upper_bounds = np.array([1200, 1200, 0.55, 0.55])
    integer_indices = [1]
    optimizer = MixedIntegerOptimizer(obj_func=G05, constraints_fun= G05_cons,lower=lower_bounds, upper=upper_bounds, max_eval=1000, minimize=True, verbose=True, integer_indices = integer_indices)
