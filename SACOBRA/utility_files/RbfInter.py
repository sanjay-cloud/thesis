...
#import pandas as pd
#U = np.genfromtxt('Fres.csv')
#xp = np.array(pd.read_csv("A.csv").values)
...
def trainCubicRBF(xp, U, ptail=True, squares=False, rho=0.0):
...
def interpRBF(x,rbfModel):
    ...
    if rbfModel['type']=='CUBIC':
        ...
        raise ValueError('CUBIC NOT COHSEN, GAUSS to be implemented')
