...
sys.path.append('..')
from utility_files.RbfSurrMixInt import trainCubicRBF
from utility_files.RbfSurrMixInt import predictRBFinter
from utility_files.RbfSurrMixInt import distLine
from utility_files.RbfSurrMixInt import interpRBF

from utility_files.MixIntEs import MixedIntegerOptimizer
...
#        global constraintSurrogates
    ...
    printP = True
    results_arr = []
    optimizer = MixedIntegerOptimizer(subProb2,constraints_fun = gCOBRA,lower=cobra['lower'], upper=cobra['upper'], max_eval=10000, minimize=True, integer_indices=cobra['integer_indices'], verbose=False)
    ...
    while n < cobra['feval']:
        ...
        # print("a length", len(cobra['A']))
        # print("fres length", len(cobra['Fres']))
        ...
        if cobra['seqOptimizer']=='COBYLA':            
            ...
            initial_pop = optimizer.generate_pop( pop_size=100, integer_indices=cobra["integer_indices"])
            sol, fit = optimizer.optimize(initial_pop)
            # print("sol",  sol, fit)
            subMin = {}
            subMin['feval'] = 1000
            subMin['x'] = sol
        ...
        xNew[cobra['integer_indices']] = np.round(xNew[cobra['integer_indices']])
        xNewTemp = xNew
        ...
        if(cobra['maxViol'][cobra['ibest']] <= 0):
            results_arr.append( ["fbest"])
        
        cobra = updateSaveCobra(cobra, xNew, feas, feasPred, feval, optimizerConvergence, 
                                predY, predVal, subMin, sigmaD, penaF, gama, EPS)
    save_results(cobra['fName'], cobra['cobraSeed'], results_arr)
    return(cobra)

def save_results(fName, cobraSeed, results_arr):
    filename = fName+"_"+str(cobraSeed)    
    fold = "without_plog"
    path = '../results/'+fold+'/rbf_mies/'+ fName

    if(not os.path.exists(path)):
        os.makedirs(path, exist_ok = True)

    np.savetxt(path +"/"+ filename, results_arr)
